...
def reshape_to_image(hvg_matrix):
    if hvg_matrix.shape[1] != 1024:
        raise ValueError("Input matrix must have 1024 features to reshape into 32x32.")
    return hvg_matrix.reshape(-1, 32, 32)


# Function to reshape (cells, 32, 32) back to (cells, 1024)
def reshape_to_vector(image_matrix):
    if image_matrix.shape[1:] != (32, 32):
        raise ValueError("Input matrix must have shape (cells, 32, 32) to reshape back to 1024 features.")
    return image_matrix.reshape(-1, 1024)


# 读入后顺便数据预处理
def preprocess_h5ad(adata, target_genes=1024):
...
class SingleCellDataset(Dataset):
    def __init__(self, data_path, labels_path):
        ...
        adata = preprocess_h5ad(adata, target_genes=1024)
...
data_loader = get_single_cell_loader(
    data_path='../../data/He_Long_Bone/He_Long_Bone.h5ad',
...
class DiffusionUNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, time_embed_dim=128):
        ...
        self.time_embed = nn.Sequential(
            ...
            nn.Linear(time_embed_dim, time_embed_dim),
            nn.ReLU(),
            nn.Linear(time_embed_dim, in_channels * 32 * 32),  # 生成与输入图像匹配的嵌入形状
        )
...
class Text2Vec(nn.Module):
    def __init__(self, max_batches=1000, embedding_dim=64, target_size=(32, 32)):
        """
...
# 定义 text2vec 模型
text2vec = Text2Vec(max_batches=len(data_loader), embedding_dim=64, target_size=(32, 32)).to(config['device'])
