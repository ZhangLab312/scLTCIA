...
def reshape_to_image(hvg_matrix):
    if hvg_matrix.shape[1] != 4096:
        raise ValueError("Input matrix must have 4096 features to reshape into 64x64.")
    return hvg_matrix.reshape(-1, 64, 64)


# Function to reshape (cells, 32, 32) back to (cells, 1024)
def reshape_to_vector(image_matrix):
    if image_matrix.shape[1:] != (64, 64):
        raise ValueError("Input matrix must have shape (cells, 64, 64) to reshape back to 4096 features.")
    return image_matrix.reshape(-1, 4096)


# 读入后顺便数据预处理
def preprocess_h5ad(adata, target_genes=4096):
...
data_loader = get_single_cell_loader(
    data_path='../../data/scrna_data/Inter/He_Long_Bone_4096.h5ad',
...
class DiffusionUNet(nn.Module):
    def __init__(self, in_channels=1, out_channels=1, time_embed_dim=128):
        ...
        self.time_embed = nn.Sequential(
            ...
            nn.Linear(time_embed_dim, time_embed_dim),
            nn.ReLU(),
            nn.Linear(time_embed_dim, in_channels * 64 * 64),  # 生成与输入图像匹配的嵌入形状
        )
...
class Text2Vec(nn.Module):
    def __init__(self, max_batches=1000, embedding_dim=64, target_size=(64, 64)):
        """
...
# 定义 text2vec 模型
text2vec = Text2Vec(max_batches=len(data_loader), embedding_dim=64, target_size=(64, 64)).to(config['device'])
