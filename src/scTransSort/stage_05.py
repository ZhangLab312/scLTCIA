...
from src.test_sctranssort.model_sctranssort import *
...
def reshape(X, patch_size):
    """
    Reshape the input tensor from (batch, seq_len) to (batch, patch_size, patch_size).

    Args:
        X (torch.Tensor or np.ndarray): Input data of shape (batch, seq_len).
        patch_size (int): Desired patch size for the reshaped tensor.

    Returns:
        torch.Tensor: Reshaped tensor of shape (batch, patch_size, patch_size).
    """
    ...
    assert isinstance(X, torch.Tensor), "Input must be a PyTorch tensor or a NumPy array."

    batch, seq_len = X.shape

    # Ensure seq_len can be reshaped into a square matrix of size (patch_size x patch_size)
    assert seq_len == patch_size * patch_size, \
        f"seq_len ({seq_len}) must be equal to patch_size^2 ({patch_size ** 2})."

    # Reshape the tensor
    reshaped_X = X.contiguous().view(batch, patch_size, patch_size)
    return reshaped_X
...
def load_and_preprocess_data(
    ...
    X_train = reshape(X_train, patch_size=patch_size)
    ...
    X_test = reshape(X_test, patch_size=patch_size)
...
def train(model_1, model_2, train_loader, optimizer_1, optimizer_2, device):
    ...
    for data, target, incremental_mask in tqdm(train_loader, desc="Training", leave=False):
        ...
        kd_loss, _, _, _ = distillation_loss(
            ...
            alpha=0.5, beta=0.8, gamma=0.1, temperature=2.0
        )
...
class Model(nn.Module):
    def __init__(
            self, num_genes, num_cell_types,
            img_size0=32, img_size1=32, patch_size=32, embed_dim=1024, nhead=64, dropout=0.3
    ):
        super(Model, self).__init__()
        self.sctranssort = scTransSort(
            img_size0=img_size0, img_size1=img_size1, patch_size=patch_size,
            embed_dim=embed_dim, nhead=nhead, dropout=dropout
        )
...
if __name__ == "__main__":
    # 参数
    file_path = '../../../data/scrna_data/Inter/He_Long_Bone_1024.h5ad'
    cell_type_column = 'cell_type1'
    batch_size = 256
    checkpoint_path = './pth/stage_04_model.pth'
    save_path = './pth/stage_05_model.pth'
    ...
    # 初始化模型、损失函数和优化器
    model_1 = Model(
        num_genes=1024, num_cell_types=num_cell_types,
        img_size0=32, img_size1=32, patch_size=32, embed_dim=1024, nhead=64, dropout=0.3
    ).to(device)
    model_2 = Model(
        num_genes=1024, num_cell_types=num_cell_types,
